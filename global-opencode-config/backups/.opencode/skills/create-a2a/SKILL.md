...
- Template repo: `/Users/jeremy/dev/SIN-Solver/a2a/template-repo/A2A-SIN-Agent-Template`
- Scaffold script: `/Users/jeremy/dev/SIN-Solver/scripts/create-sin-a2a-agent.mjs`
- Fleet validator: `/Users/jeremy/dev/SIN-Solver/scripts/validate-sin-a2a-fleet.mjs`
...
- Control-plane projection generator: `/Users/jeremy/dev/SIN-Solver/scripts/alpha/generate-control-plane-registry-projection.mjs`
- Control-plane projection artifact: `/Users/jeremy/dev/SIN-Solver/Docs/operations/a2a-control-plane-projection.json`
- Dedicated-doc sync: `/Users/jeremy/dev/SIN-Solver/scripts/sync-a2a-google-doc.mjs`
- HF VM runtime SSOT bundle: `/Users/jeremy/dev/SIN-Solver/a2a/team-infratructur/A2A-SIN-Server/hf-docs/`
...
4. **The OpenCode CLI Standard (CRITICAL):** EVERY general A2A agent MUST be equipped with the `opencode` CLI as its brain. You MUST use the model `openai/gpt-5.4` and explicitly append `--fallback opencode/minimax-m2.5-free`. Only specialized agents (e.g., Voice) may deviate from this.
...
- **The Intelligence Core:** You must wire the agent's `runtime.ts` to call `opencode run "..." --model openai/gpt-5.4 --fallback opencode/minimax-m2.5-free`. Do not hardcode direct `fetch` or SDK calls to OpenAI/Gemini APIs unless absolutely necessary for streaming or specialized multimodal tasks.
...
```bash
cd /Users/jeremy/dev/SIN-Solver && npm run sync:a2a:control-plane-projection
```
