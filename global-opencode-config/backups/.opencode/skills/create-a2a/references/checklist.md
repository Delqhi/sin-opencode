...
- a live HF health URL for readiness verification; the front-door verifier must be run with `--health-url <public>/health`
...
- Google Docs document exists and is synced from `A2A-CARD.md`
- repo declares inbound platform registry and PR watcher contract
- n8n inbound intake workflow exists or is explicitly documented as `manual-none`
...
cd /Users/jeremy/dev/SIN-Solver && node scripts/sync-a2a-google-doc.mjs --agent <slug>
...
- HF-hosted agents also answer `/health`, `/.well-known/agent-card.json`, and `/a2a/v1` from the public host
...
- `a2a-verify-hf-vm-readiness.mjs --health-url <public>/health` passes with live endpoint checks
